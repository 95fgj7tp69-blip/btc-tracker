...
const DARK = {
  ...
  text: "#fff", textSub: "#aaa", textMuted: "#777", textFaint: "#555",
  input: "#111", inputBorder: "#222", navBg: "rgba(5,5,5,0.97)", divider: "#1a1a1a",
};

const LIGHT = {
  bg: "#f2f2f7", surface: "#fff", border: "#e0e0e0",
  text: "#000", textSub: "#3a3a3a", textMuted: "#555", textFaint: "#888",
  input: "#f5f5f5", inputBorder: "#d0d0d0", navBg: "rgba(242,242,247,0.97)", divider: "#e0e0e0",
};
    ...
    <div style={{ margin: "0 12px 12px", background: T.surface, border: `1px solid ${T.border}`, borderRadius: 20, overflow: "hidden" }}>
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{ fontSize: 36, fontWeight: 700, color: T.text, letterSpacing: "-0.02em", lineHeight: 1.1 }}>
          <span style={{ fontSize: 22, fontWeight: 500, color: T.textMuted, marginRight: 3 }}>CHF</span>{fmtChf(portfolioChf)}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6, marginBottom: 20 }}>
          <span style={{ color: isNeg ? "#ef4444" : "#22c55e", fontSize: 14, fontWeight: 500 }}>{isNeg ? "↓" : "↑"} {fmtChf(Math.abs(pnlChf))} CHF ({isNeg ? "" : "+"}{pnlPct.toFixed(2)}%)</span>
      ...
      <div style={{ height: 140, position: "relative" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 50, left: 0, bottom: 0 }}>
            ...
            <Tooltip contentStyle={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13 }} labelStyle={{ color: T.textMuted }} itemStyle={{ color: T.text, fontWeight: 500 }} formatter={(v) => [`CHF ${fmtChf(v, 0)}`, ""]} />
  ...
  return (
    <div style={{ margin: "0 12px 12px", background: T.surface, border: `1px solid ${T.border}`, borderRadius: 20, overflow: "hidden" }}>
      <div style={{ padding: "18px 20px 12px" }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 700, color: T.text, letterSpacing: "-0.02em" }}>CHF {fmtChf(btcChf, 0)}</div>
...
function BottomNav({ view, setView, onAdd, T }) {
  const btn = (id, icon, label) => (
    <button onClick={() => setView(id)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
      <span style={{ fontSize: 19, color: view === id ? "#f7931a" : T.textFaint }}>{icon}</span>
      <span style={{ fontSize: 10, color: view === id ? "#f7931a" : T.textFaint, fontWeight: view === id ? 600 : 400 }}>{label}</span>
    </button>
  );
  return (
    <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: T.navBg, backdropFilter: "blur(20px)", borderTop: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-around", padding: "10px 16px calc(16px + env(safe-area-inset-bottom))" }}>
