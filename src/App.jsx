  ...
  return (
    <div style={{ background: "#0d0d0d", border: "1px solid #1f1f1f", borderRadius: 20, padding: "20px 16px 16px", marginBottom: 12 }}>
      ...
      <div style={{ marginTop: 10, padding: "10px 14px", background: "#111", borderRadius: 8, border: "1px solid #1a1a1a", display: "flex", justifyContent: "space-between", alignItems: "center", minHeight: 40 }}>
        {tooltip ? (<><span style={{ color: "#777", fontSize: 13 }}>{tooltip.label}</span><span style={{ color: "#fff", fontSize: 14, fontWeight: 500 }}>CHF {fmt(tooltip.price, 0)}</span></>) : (<><span style={{ color: "#444", fontSize: 13 }}>Finger ziehen zum Ablesen</span><span style={{ color: "#666", fontSize: 13 }}>CHF {fmt(currentChf, 0)}</span></>)}
      </div>
    </div>
  );
}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ color: isAbove ? "#22c55e" : "#ef4444", fontSize: 22, fontWeight: 300 }}>{isAbove ? "+" : ""}{fmt(diff, 0)}<span style={{ fontSize: 14, marginLeft: 4, opacity: 0.7 }}>CHF</span></div>
          ...
          <div style={{ background: isAbove ? "rgba(34,197,94,0.06)" : "rgba(239,68,68,0.06)", border: `1px solid ${isAbove ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)"}`, borderRadius: 10, padding: "10px 12px" }}>
            {isAbove ? (<><div style={{ color: "#888", fontSize: 12, letterSpacing: "0.06em", marginBottom: 4 }}>IM GEWINN SEIT</div><div style={{ color: "#22c55e", fontSize: 17 }}>CHF {fmt(avgChf, 0)}</div></>) : (<><div style={{ color: "#888", fontSize: 12, letterSpacing: "0.06em", marginBottom: 4 }}>BTC MUSS STEIGEN UM</div><div style={{ color: "#ef4444", fontSize: 17 }}>+{toBreakEvenPct.toFixed(1)}%</div><div style={{ color: "#777", fontSize: 13, marginTop: 3 }}>auf CHF {fmt(avgChf, 0)}</div></>)}
          </div>
        </div>
      </div>
      <div style={{ marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ color: "#555", fontSize: 12 }}>Einstand CHF {fmt(avgChf, 0)}</span><span style={{ color: "#555", fontSize: 12 }}>Aktuell CHF {fmt(currentChf, 0)}</span></div>
    ...
    <div style={{ background: "#0d0d0d", border: "1px solid #1f1f1f", borderRadius: 20, padding: "20px 16px 20px", marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", background: "#111", borderRadius: 10, padding: 3, marginBottom: 16, gap: 3 }}>
        {[["chf", "Betrag (CHF)"], ["btc", "Menge (BTC)"]].map(([m, label]) => (
          <button key={m} onClick={() => { setMode(m); setInput(""); }} style={{ flex: 1, padding: "10px 0", borderRadius: 8, cursor: "pointer", fontSize: 14, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: mode === m ? "#1f1f1f" : "transparent", color: mode === m ? "#fff" : "#666", border: mode === m ? "1px solid #2a2a2a" : "1px solid transparent", fontWeight: mode === m ? 500 : 400 }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: hasInput ? 14 : 0 }}>
        <div style={{ background: "#111", borderRadius: 12, padding: "14px 12px" }}>
          <div style={{ color: "#777", fontSize: 12, letterSpacing: "0.06em", marginBottom: 6 }}>AKTUELLER EINSTAND</div>
          <div style={{ color: "#fff", fontSize: 18, fontWeight: 300 }}>CHF {fmt(avgChf, 0)}</div>
          <div style={{ color: "#666", fontSize: 13, marginTop: 4 }}>${fmt(avgChf / usdChf, 0)}</div>
        </div>
        <div style={{ background: hasInput ? (newAvgChf < avgChf ? "rgba(34,197,94,0.07)" : "rgba(239,68,68,0.07)") : "#111", border: hasInput ? `1px solid ${newAvgChf < avgChf ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)"}` : "1px solid transparent", borderRadius: 12, padding: "14px 12px", transition: "background 0.3s" }}>
          <div style={{ color: "#777", fontSize: 12, letterSpacing: "0.06em", marginBottom: 6 }}>NEUER EINSTAND</div>
          <div style={{ color: hasInput ? (newAvgChf < avgChf ? "#22c55e" : "#ef4444") : "#555", fontSize: 18, fontWeight: 300 }}>{hasInput ? `CHF ${fmt(newAvgChf, 0)}` : "—"}</div>
          <div style={{ color: "#666", fontSize: 13, marginTop: 4 }}>{hasInput ? `$${fmt(newAvgUsd, 0)}` : ""}</div>
        </div>
      </div>
      {hasInput && (
        <div style={{ background: newAvgChf < avgChf ? "rgba(34,197,94,0.05)" : "rgba(239,68,68,0.05)", border: `1px solid ${newAvgChf < avgChf ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)"}`, borderRadius: 12, padding: "14px 16px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[{ label: "EINSTAND Δ", val: `${avgDrop > 0 ? "+" : ""}${avgDrop.toFixed(1)}%`, color: avgDrop < 0 ? "#22c55e" : "#ef4444" }, { label: "GEKAUFT", val: `${fmtBtc(newBtc)} BTC`, color: "#fff" }, { label: "GESAMT BTC", val: fmtBtc(newTotalBtc), color: "#fff" }].map(({ label, val, color }) => (
              <div key={label} style={{ textAlign: "center" }}><div style={{ color: "#666", fontSize: 11, letterSpacing: "0.06em", marginBottom: 5 }}>{label}</div><div style={{ color, fontSize: 14, fontWeight: 500 }}>{val}</div></div>
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ padding: "0 16px 16px" }}>
      <div style={{ background: "linear-gradient(145deg,#111 0%,#0d0d0d 100%)", border: "1px solid #1f1f1f", borderRadius: 20, padding: "28px 24px 24px", marginBottom: 12, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ color: "#888", fontSize: 13, letterSpacing: "0.06em", marginBottom: 10 }}>PORTFOLIO WERT</div>
        <div style={{ fontSize: 46, fontWeight: 300, letterSpacing: "-0.03em", lineHeight: 1, marginBottom: 16 }}>
          <span style={{ color: "#777", fontSize: 20, marginRight: 6 }}>CHF</span>
          <span style={{ color: "#fff" }}>{fmt(portfolioChf)}</span>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <span style={{ background: neg ? "rgba(239,68,68,0.15)" : "rgba(34,197,94,0.15)", color: neg ? "#ef4444" : "#22c55e", fontSize: 14, fontWeight: 500, padding: "5px 12px", borderRadius: 8 }}>{neg ? "" : "+"}{fmt(pnlChf)} CHF</span>
          <span style={{ background: neg ? "rgba(239,68,68,0.1)" : "rgba(34,197,94,0.1)", color: neg ? "#ef4444" : "#22c55e", fontSize: 14, padding: "5px 12px", borderRadius: 8 }}>{neg ? "" : "+"}{pnlPct.toFixed(2)}%</span>
        </div>
      </div>
      ...
      {[{ label: "AKTUELLER KURS", left: fmt(btcUsd, 0), leftSub: "USD", right: fmt(btcChf, 0), rightSub: "CHF", mb: 10 }, { label: "Ø EINSTANDSPREIS", left: fmt(avgUsd, 0), leftSub: "USD", right: fmt(avgChf, 0), rightSub: "CHF", mb: 0 }].map(({ label, left, leftSub, right, rightSub, mb }) => (
        <div key={label} style={{ background: "#0d0d0d", border: "1px solid #1f1f1f", borderRadius: 16, padding: "18px 16px", marginBottom: mb }}>
          <div style={{ color: "#888", fontSize: 13, letterSpacing: "0.06em", marginBottom: 12 }}>{label}</div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ color: "#fff", fontSize: 22, fontWeight: 300 }}>{left}</div><div style={{ color: "#888", fontSize: 13, marginTop: 4 }}>{leftSub}</div></div>
            <div style={{ width: 1, background: "#1f1f1f", alignSelf: "stretch" }} />
            <div style={{ textAlign: "right" }}><div style={{ color: "#fff", fontSize: 22, fontWeight: 300 }}>{right}</div><div style={{ color: "#888", fontSize: 13, marginTop: 4 }}>{rightSub}</div></div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div style={{ padding: "0 16px 100px" }}>
      <div style={{ background: "#0d0d0d", border: "1px solid #1f1f1f", borderRadius: 20, padding: "20px 16px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", height: 180, position: "relative" }}>
          {steps.map(v => (<div key={v} style={{ position: "absolute", left: 32, right: 0, bottom: `${(v / max) * 100}%`, borderTop: "1px solid #161616" }}><span style={{ position: "absolute", left: -30, bottom: 2, color: "#444", fontSize: 10 }}>{v >= 1000 ? `${v / 1000}k` : v}</span></div>))}
          <div style={{ flex: 1, display: "flex", gap: 16, alignItems: "flex-end", paddingLeft: 36, height: "100%" }}>
            {bars.map(({ label, value, color }) => (
              <div key={label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, height: "100%", justifyContent: "flex-end" }}>
                <div style={{ color: "#555", fontSize: 11 }}>{fmt(value, 0)}</div>
...
function TransactionModal({ onClose, onSave, editTx }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#0a0a0a", borderTop: "1px solid #1f1f1f", borderRadius: "20px 20px 0 0", padding: "12px 20px 40px", width: "100%", maxWidth: 420, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <Field label="TYP">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {Object.entries(TYPE_META).map(([t, m]) => (<button key={t} onClick={() => set("type", t)} style={{ padding: "12px 0", borderRadius: 10, background: form.type === t ? m.bg : "#111", border: `1px solid ${form.type === t ? m.color + "55" : "#222"}`, color: form.type === t ? m.color : "#666", cursor: "pointer", fontSize: 15, fontWeight: 500, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}><span style={{ fontSize: 19 }}>{m.icon}</span>{m.label}</button>))}
          </div>
        </Field>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 12, marginTop: 8 }}>
          <button onClick={onClose} style={{ padding: "15px 0", background: "#111", border: "1px solid #222", color: "#777", borderRadius: 12, cursor: "pointer", fontSize: 15, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>Abbrechen</button>
          <button onClick={handleSave} disabled={saving} style={{ padding: "15px 0", background: saving ? "#333" : TYPE_META[form.type].color, border: "none", color: form.type === "buy" ? "#000" : "#fff", borderRadius: 12, cursor: "pointer", fontSize: 15, fontWeight: 600, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 0", borderBottom: "1px solid #0e0e0e" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          ...
          <div style={{ color: m.color, fontSize: 15 }}>{tx.type === "transfer" ? (tx.fee > 0 ? `−${tx.fee} BTC` : "—") : `CHF ${fmt(tx.chf)}`}</div>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
          <div style={{ color: "#555", fontSize: 13 }}>{tx.date}{tx.note ? ` · ${tx.note}` : ""}</div>
          {tx.fee > 0 && tx.type !== "transfer" && <div style={{ color: "#444", fontSize: 12 }}>Geb. CHF {fmt(tx.fee)}</div>}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 420, margin: "0 auto", minHeight: "100vh", background: "#000" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 20px 12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, background: "#f7931a", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#000" }}>₿</div>
            <div>
              <div style={{ fontSize: 16, fontWeight: 500, letterSpacing: "-0.01em" }}>Portfolio</div>
              {lastUpdated && <div style={{ fontSize: 11, color: "#444", marginTop: 1 }}>Aktualisiert {fmtTime(lastUpdated)}</div>}
            </div>
          </div>
          <button onClick={() => { fetchPrice(); fetchChart(); }} style={{ background: "#111", border: "1px solid #1f1f1f", borderRadius: 8, color: loading ? "#f59e0b" : "#666", cursor: "pointer", fontSize: 14, padding: "7px 14px", display: "flex", alignItems: "center", gap: 6, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
            <span style={{ display: "inline-block", animation: loading ? "spin 1s linear infinite" : "none" }}>↻</span>
            <span style={{ fontSize: 13 }}>{loading ? "..." : `$${fmt(btcUsd, 0)}`}</span>
          </button>
        </div>
        ...
        {dbLoading ? (
          ...
          <>
            {view === "dashboard" && (
              <div style={{ overflowY: "auto", maxHeight: "calc(100vh - 130px - env(safe-area-inset-bottom))", WebkitOverflowScrolling: "touch" }}>
            ...
            {view === "transactions" && (
              <div style={{ padding: "0 16px", overflowY: "auto", maxHeight: "calc(100vh - 130px - env(safe-area-inset-bottom))", WebkitOverflowScrolling: "touch" }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                  {[["all", "Alle"], ...Object.entries(TYPE_META).map(([k, v]) => [k, v.label])].map(([id, label]) => (
                    <button key={id} onClick={() => setTxFilter(id)} style={{ padding: "7px 16px", borderRadius: 20, cursor: "pointer", fontSize: 13, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: txFilter === id ? "#fff" : "#111", color: txFilter === id ? "#000" : "#666", border: `1px solid ${txFilter === id ? "#fff" : "#1f1f1f"}`, fontWeight: txFilter === id ? 500 : 400 }}>{label}</button>
                  ))}
                </div>
        ...
        <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 420, background: "rgba(0,0,0,0.95)", backdropFilter: "blur(16px)", borderTop: "1px solid #111", display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", padding: "10px 32px calc(20px + env(safe-area-inset-bottom))", gap: 12 }}>
          <button onClick={() => setView("dashboard")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, color: view === "dashboard" ? "#fff" : "#444", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", justifySelf: "end" }}>
            <span style={{ fontSize: 20 }}>◈</span><span style={{ fontSize: 11, letterSpacing: "0.04em" }}>Dashboard</span>
          </button>
          <button onClick={() => { setEditTx(null); setShowModal(true); }} style={{ width: 56, height: 56, borderRadius: 16, background: "#fff", border: "none", color: "#000", cursor: "pointer", fontSize: 30, fontWeight: 300, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 24px rgba(255,255,255,0.12)", lineHeight: 1 }}>+</button>
          <button onClick={() => setView("transactions")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, color: view === "transactions" ? "#fff" : "#444", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", justifySelf: "start" }}>
            <span style={{ fontSize: 20 }}>≡</span><span style={{ fontSize: 11, letterSpacing: "0.04em" }}>Verlauf</span>
          </button>
        </div>
      </div>
