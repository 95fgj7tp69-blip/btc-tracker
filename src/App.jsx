      ...
      <div style={{ height: 150 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData || []} margin={{ top: 5, right: 16, left: 0, bottom: 20 }}>
            ...
            <YAxis yAxisId="portfolio" hide domain={["auto", "auto"]} />
            <YAxis yAxisId="invested" hide domain={["auto", "auto"]} orientation="right" />
            <Tooltip
            ...
            />
            <Line yAxisId="invested" type="stepAfter" dataKey="invested" stroke="#f7931a" strokeWidth={2} dot={false} activeDot={{ r: 3 }} />
            {chartData?.[0]?.portfolio !== undefined ? (
              <Line yAxisId="portfolio" type="monotone" dataKey="portfolio" stroke={isNeg ? "#ef4444" : "#22c55e"} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: isNeg ? "#ef4444" : "#22c55e" }} />
            ) : (
              <Line yAxisId="portfolio" type="monotone" dataKey="today" stroke={isNeg ? "#ef4444" : "#22c55e"} strokeWidth={0}
                dot={(props) => {
...
function SettingsView({ darkMode, setDarkMode, T, transactions, userEmail, onLogout, currency = "CHF", setCurrency, usdChf = 0.9, eurUsd = 0.92, btcChf = 0, btcUsd = 0, onResetOnboarding, onImport, costMethod = "FIFO", setCostMethod }) {
  ...
  return (
    ...
    <div style={{ padding: "8px 16px", overflowY: "auto", maxHeight: "calc(100vh - 80px - env(safe-area-inset-bottom))", paddingBottom: 100 }}>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden" }}>
        {[{ label: "Version", value: "1.14.2" }, { label: "Datenbank", value: "Supabase" }, { label: "Kurs-API", value: "CoinGecko" }].map(({ label, value }, i, arr) => (
