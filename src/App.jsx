...
const setPremiumStatus = (v) => {
  try { localStorage.setItem("isPremium", String(v)); } catch {}
};
  ...
  const slides = [
    { title: slidesData[0].title, text: slidesData[0].text, svg: (
        <svg viewBox="0 0 280 200" width="260" style={{ display: "block" }}>
          <circle cx="140" cy="88" r="75" fill="#1a1a1a" stroke="#2a2a2a" strokeWidth="1" opacity="0.6"/>
          <circle cx="140" cy="88" r="95" fill="none" stroke="#f7931a" strokeWidth="0.5" strokeOpacity="0.2"/>
          <image href={TRACKOSHI_ICON} x="90" y="38" width="100" height="100"/>
          <text x="22" y="70" fontSize="11" fill="#f7931a" opacity="0.6" fontFamily="monospace">+89.6%</text>
          <text x="196" y="50" fontSize="10" fill="#777" fontFamily="monospace">CHF 87'914</text>
          <text x="193" y="122" fontSize="10" fill="#f7931a" opacity="0.45" fontFamily="monospace">1.3799 BTC</text>
          <text x="18" y="126" fontSize="10" fill="#777" fontFamily="monospace">↑ +49'032</text>
    ...
    { title: slidesData[1].title, text: slidesData[1].text, svg: (
        <svg viewBox="0 0 280 210" width="270" style={{ display: "block" }}>
          <rect x="20" y="10" width="240" height="100" rx="14" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="36" y="32" fontSize="10" fill="#777">BTC Kurs</text>
          ...
          <rect x="20" y="120" width="115" height="78" rx="12" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="32" y="140" fontSize="9" fill="#777">Gesamtwert</text>
          ...
          <rect x="145" y="120" width="115" height="78" rx="12" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="157" y="140" fontSize="9" fill="#777">BTC Bestand</text>
          <text x="157" y="158" fontSize="16" fontWeight="700" fill="#fff" letterSpacing="-0.3">1.3799</text>
          <text x="157" y="173" fontSize="9" fill="#777">BTC</text>
          <text x="157" y="188" fontSize="9" fill="#777">Einstand CHF 35'693</text>
        </svg>
    )},
    { title: slidesData[2].title, text: slidesData[2].text, svg: (
        <svg viewBox="0 0 280 210" width="270" style={{ display: "block" }}>
          <rect x="20" y="10" width="240" height="115" rx="14" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="36" y="30" fontSize="9" fill="#777">Break-even Analyse</text>
          ...
          <text x="165" y="82" fontSize="9" fill="#777">seit Einstand</text>
          <text x="36" y="110" fontSize="9" fill="#444">Einstand CHF 35'693</text>
          <text x="155" y="110" fontSize="9" fill="#444">Aktuell CHF 63'693</text>
          <rect x="20" y="135" width="240" height="65" rx="12" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="36" y="152" fontSize="9" fill="#777">Kursverlauf vs. Einstand</text>
    ...
    { title: slidesData[4].title, text: slidesData[4].text, svg: (
        <svg viewBox="0 0 280 210" width="260" style={{ display: "block" }}>
          <path d="M 140 15 L 205 42 L 205 112 C 205 150 140 178 140 178 C 140 178 75 150 75 112 L 75 42 Z" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1.5"/>
          ...
          <rect x="14" y="58" width="56" height="22" rx="11" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="42" y="72" fontSize="9" fill="#777" textAnchor="middle">CSV Export</text>
          <rect x="210" y="58" width="56" height="22" rx="11" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="238" y="72" fontSize="9" fill="#777" textAnchor="middle">DSGVO ✓</text>
          <rect x="14" y="98" width="56" height="22" rx="11" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="42" y="112" fontSize="9" fill="#777" textAnchor="middle">Löschbar</text>
          <rect x="210" y="98" width="56" height="22" rx="11" fill="#2c2c2e" stroke="#3a3a3c" strokeWidth="1"/>
          <text x="238" y="112" fontSize="9" fill="#777" textAnchor="middle">Privat ✓</text>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0a0a0a", zIndex: 500, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: "100%", maxWidth: 360 }}>
        ...
        <div style={{ color: "#aaa", fontSize: 17, textAlign: "center", lineHeight: 1.65, maxWidth: 290 }}>{s.text}</div>
      </div>
...
// ── Settings ──────────────────────────────────────────────────────────────────
function SettingsView({ darkMode, setDarkMode, T, transactions, userEmail, onLogout, currency = "CHF", setCurrency, usdChf = 0.9, eurUsd = 0.92, btcChf = 0, btcUsd = 0, onResetOnboarding, onImport, costMethod = "FIFO", setCostMethod, language, setLanguage, secondaryCurrency = "none", setSecondaryCurrency, fontScale = "M", setFontScale, showFearGreed = false, setShowFearGreed, showMarketChart = true, setShowMarketChart, showPositionCard = true, setShowPositionCard, isPremium = false, onUpgrade, onRestorePurchases }) {
  const t = tr(translations, language);
  const [showPrivacy, setShowPrivacy] = useState(false);
...
export default function App() {
  ...
  const handleSave = async (form) => {
    if (form.id && transactions.find(t => t.id === form.id)) {
      ...
      if (!isPremium && transactions.length >= FREE_TX_LIMIT) {
        setShowModal(false);
        openPaywall("limit");
        return;
      }
  ...
  const handleImportTransactions = async (rows) => {
    ...
    for (const row of rows) {
      // Free-Limit-Check beim Import: stoppe wenn Limit erreicht
      if (!isPremium && (transactions.length + count) >= FREE_TX_LIMIT) {
        break;
      }
      try {
          ...
          <>
            ...
            {view === "verlauf" && (
              <div style={{ ...scrollStyle, padding: "0 16px" }}>
                {/* Free-Limit-Banner */}
                {!isPremium && transactions.length >= FREE_TX_LIMIT && (
              ...
              <div style={{ ...scrollStyle, padding: "0 16px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10, marginBottom: 20 }}>
                  ...
                  <button
                    ...
                    style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: 160, padding: "16px 14px", background: "#fff8f0", border: `1px solid rgba(247,147,26,0.15)`, borderRadius: 18, cursor: aiLoading || totalBtc === 0 ? "not-allowed" : "pointer", fontFamily: "inherit", textAlign: "left", opacity: aiLoading || totalBtc === 0 ? 0.5 : 1, position: "relative" }}
                  >
                    {!isPremium && <div style={{ position: "absolute", top: 10, right: 10, fontSize: 16, color: "#f7931a" }}>🔒</div>}
                    <div style={{ width: 40, height: 40, borderRadius: 11, background: "#f7931a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 22 }}>📊</div>
                    <div>
                  ...
                  <button
                    ...
                    disabled={aiLoading}
                    style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: 160, padding: "16px 14px", background: "#fff8f0", border: `1px solid rgba(247,147,26,0.15)`, borderRadius: 18, cursor: aiLoading ? "not-allowed" : "pointer", fontFamily: "inherit", textAlign: "left", opacity: aiLoading ? 0.5 : 1, position: "relative" }}
                  >
                    {!isPremium && <div style={{ position: "absolute", top: 10, right: 10, fontSize: 16, color: "#f7931a" }}>🔒</div>}
                    <div style={{ width: 40, height: 40, borderRadius: 11, background: "#f7931a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 22 }}>🌐</div>
                    <div>
